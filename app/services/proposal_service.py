...
class ProposalService:
    ...
    def create_proposal(
        self, proposal_data: CreateProposalRequest, user_id: UUID
    ) -> Proposal:
    ...
    def get_user_proposals(
        ...
        return self.repository.get_by_user(
            user_id=user_id, page=page, size=size, status=status
        )
    ...
    def get_property_proposals(
        ...
        return self.repository.get_by_property(
            property_id=property_id, page=page, size=size, status=status
        )
    ...
    def get_received_proposals(
        ...
        return self.repository.get_by_property_owner(
            owner_id=owner_id, page=page, size=size, status=status
        )

    def accept_proposal(
        self, proposal_id: UUID, owner_id: UUID, response_message: Optional[str] = None
    ) -> Proposal:
    ...
    def withdraw_proposal(self, proposal_id: UUID, user_id: UUID) -> Proposal:
    ...
    def delete_all_proposals_for_owner_properties(self, owner_id: UUID) -> int:
        ...
        """
        try:
            deleted_count = self.repository.delete_all_proposals_for_owner_properties(
                owner_id
            )
