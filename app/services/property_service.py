...
class PropertyService:
    ...
    def create_house(self, house_data: CreateHouseRequest, owner_id: str) -> Property:
        ...
        try:
            ...
            logger.info(
                f"House created successfully with {len(house_data.images)} images: {created_house.id} by owner {owner_id}"
            )
    ...
    def create_apartment(
        self, apartment_data: CreateApartmentRequest, owner_id: str
    ) -> Property:
        """
        ...
        try:
            ...
            logger.info(
                f"Apartment created successfully with {len(apartment_data.images)} images: {created_apartment.id} by owner {owner_id}"
            )
    ...
    def get_all_properties(
        ...
        property_type_enum = (
            PropertyTypeEnum(property_type.upper()) if property_type else None
        )
    ...
    def update_property(
        self, property_id: str, update_data: UpdatePropertyRequest, owner_id: str
    ) -> Property:
    ...
    def delete_property(
        self, property_id: str, user_id: str, is_admin: bool = False
    ) -> None:
        """
    ...
    def delete_all_owner_properties_permanently(self, owner_id: str) -> None:
        ...
        try:
            ...
            deleted_property_ids = self.property_repo.delete_properties_by_owner_id(
                owner_id
            )

            # 2. Delete associated images using the collected property IDs
            if deleted_property_ids:
                deleted_images_count = self.image_repo.delete_images_by_property_ids(
                    deleted_property_ids
                )
                logger.info(
                    f"Deleted {deleted_images_count} images for properties of owner {owner_id}."
                )

            logger.info(
                f"Successfully deleted {len(deleted_property_ids)} properties permanently for owner {owner_id}."
            )

        except Exception as e:
            logger.error(
                f"Error deleting all properties permanently for owner {owner_id}: {e}"
            )
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
    ...
    def delete_property_and_related_data(self, property_id: str) -> None:
        ...
        try:
            ...
            logger.info(
                f"Deleted {proposal_count} proposals for property {property_id}."
            )
            ...
            logger.error(
                f"Error during permanent deletion of property {property_id}: {e}"
            )
    ...
    def get_all_properties_admin(
        ...
        return self.property_repo.get_all_admin(
            page=page, size=size, randomize=randomize, include_inactive=include_inactive
        )

    def _process_images(
        self, property_id: str, images: List[ImageUploadRequest]
    ) -> List[PropertyImage]:
        """
        ...
        try:
            ...
            for idx, img_request in enumerate(images, start=1):
                ...
                try:
                    ...
                    raise ValueError(
                        f"Invalid base64 image data for image {idx}: {str(e)}"
                    )
                ...
                property_image = PropertyImage(
                    ...
                    display_order=img_request.display_order
                    if img_request.display_order
                    else idx,
            ...
            logger.info(
                f"Created {len(created_images)} images for property {property_id}"
            )
