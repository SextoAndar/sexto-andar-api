...
class ImageUploadRequest(BaseModel):
    ...
    content_type: str = Field(
        ..., description="Image content type (image/jpeg, image/png, image/webp)"
    )
    display_order: int = Field(
        default=1, ge=1, le=15, description="Display order (1-15)"
    )
    is_primary: bool = Field(
        default=False, description="Is this the primary/main image"
    )
    ...
    def validate_image_data(cls, v: str) -> str:
        ...
        try:
            ...
            if len(decoded) > max_size:
                raise ValueError(
                    f"Image size must not exceed 5MB (got {len(decoded) / 1024 / 1024:.2f}MB)"
                )
