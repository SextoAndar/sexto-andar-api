...
class AddressRequest(BaseModel):
    ...
    postal_code: str = Field(
        ..., min_length=8, max_length=20, description="Postal code (CEP)"
    )
...
class CreateHouseRequest(BaseModel):
    ...
    # Property attributes
    propertySize: Decimal = Field(
        ..., gt=0, description="Property size in square meters"
    )
    description: str = Field(
        ..., min_length=10, max_length=1000, description="Property description"
    )
    ...
    isSingleHouse: bool = Field(
        default=True, description="Is it a single house on the land?"
    )

    # Images (required: 1-15 images)
    images: list[ImageUploadRequest] = Field(
        ..., min_length=1, max_length=15, description="Property images (1-15 required)"
    )
...
class UpdatePropertyRequest(BaseModel):
    ...
    propertySize: Optional[Decimal] = Field(
        None, gt=0, description="Property size in square meters"
    )
    description: Optional[str] = Field(
        None, min_length=10, max_length=1000, description="Property description"
    )
    ...
    isSingleHouse: Optional[bool] = Field(
        None, description="Is it a single house on the land?"
    )
...
class PortfolioStatsResponse(BaseModel):
    ...
    total_monthly_rent_potential: Decimal = Field(
        description="Potential monthly rent income"
    )
