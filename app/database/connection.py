...
def update_enums(session):
    ...
    try:
        ...
        try:
            session.execute(
                text("ALTER TYPE roleenum ADD VALUE IF NOT EXISTS 'ADMIN';")
            )
...
def update_table_columns(session, inspector):
    ...
    try:
        ...
        if result and result[1] == "NO":  # is_nullable = 'NO'
            ...
            session.execute(
                text('ALTER TABLE accounts ALTER COLUMN "phoneNumber" DROP NOT NULL')
            )
            session.commit()
            ...
            if table_name in inspector.get_table_names():
                ...
                for index in table.indexes:
                    if index.name not in db_indexes:
                        ...
                        try:
                            ...
                            logging.warning(
                                f"⚠️  Could not create index {index.name}: {e}"
                            )
                ...
                for constraint in table.foreign_key_constraints:
                    if constraint.name not in db_foreign_keys:
                        logging.info(
                            f"🔗 Creating missing foreign key: {constraint.name}"
                        )
                        try:
                            session.execute(text(str(constraint.create(bind=engine))))
                        except Exception as e:
                            logging.warning(
                                f"⚠️  Could not create foreign key {constraint.name}: {e}"
                            )
