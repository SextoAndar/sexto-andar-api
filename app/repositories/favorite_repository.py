...
class FavoriteRepository:
    ...
    def get_by_user_and_property(
        ...
        return (
            ...
            .filter(
                and_(Favorite.idUser == user_id, Favorite.idProperty == property_id)
            )
            .first()
        )

    def get_by_user(
        self, user_id: UUID, page: int = 1, size: int = 10, active_only: bool = True
        ...
        if active_only:
            query = query.join(Property).filter(Property.is_active)
    ...
    def count_by_property(self, property_id: UUID) -> int:
        ...
        return (
            self.db.query(Favorite).filter(Favorite.idProperty == property_id).count()
        )
    ...
    def count_by_user(self, user_id: UUID) -> int:
        ...
        return self.db.query(Favorite).filter(Favorite.idUser == user_id).count()
