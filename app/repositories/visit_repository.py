...
class VisitRepository:
    ...
    def get_by_user(
        ...
        if not include_cancelled:
            query = query.filter(Visit.cancelled.is_(False))

        if not include_completed:
            query = query.filter(Visit.isVisitCompleted.is_(False))
        ...
        visits = (
            query.order_by(desc(Visit.visitDate))
            .offset((page - 1) * size)
            .limit(size)
            .all()
        )
        ...
        return (
            ...
            .filter(
                and_(
                    ...
                    Visit.visitDate > now,
                    Visit.cancelled.is_(False),
                    Visit.isVisitCompleted.is_(False),
                )
            )
    ...
    def delete_by_property_id(self, property_id: str) -> int:
        ...
        num_deleted = (
            self.db.query(Visit)
            .filter(Visit.idProperty == property_id)
