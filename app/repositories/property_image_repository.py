...
class PropertyImageRepository:
    ...
    def get_by_id(self, image_id: str) -> Optional[PropertyImage]:
        """Get image by ID"""
        return self.db.query(PropertyImage).filter(PropertyImage.id == image_id).first()
    ...
    def reorder_images(
        self, property_id: str, image_orders: dict
    ) -> List[PropertyImage]:
        """
    ...
    def set_primary_image(self, property_id: str, image_id: str) -> PropertyImage:
        ...
        self.db.query(PropertyImage).filter(
            PropertyImage.property_id == property_id, PropertyImage.is_primary
