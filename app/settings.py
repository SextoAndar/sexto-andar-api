...
class Settings:
    ...
    def __init__(self):
        ...
        self.ALLOW_ORIGINS: List[str] = (
            [o.strip() for o in origins_str.split(",")]
            if origins_str
            else self._ALLOW_ORIGINS
        )
        ...
        self.AUTH_SERVICE_URL: str = os.getenv(
            "AUTH_SERVICE_URL", "https://sexto-andar-auth-6def0cff0560.herokuapp.com/"
        ).strip()
        ...
        if not self.INTERNAL_API_SECRET:
            ...
            if self.DEBUG:
                print(
                    f"⚠️  Generated temporary INTERNAL_API_SECRET: {self.INTERNAL_API_SECRET}"
                )
