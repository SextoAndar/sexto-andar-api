...
class Property(BaseModel):
    ...
    condominiumFee = Column(Numeric(10, 2), nullable=True, default=0.00)

    commonArea = Column(Boolean, default=False, nullable=False)

    floor = Column(Integer, nullable=True)

    isPetAllowed = Column(Boolean, default=False, nullable=False)
    ...
    salesType = Column(Enum(SalesTypeEnum), nullable=False)

    propertyType = Column(Enum(PropertyTypeEnum), nullable=False)
    ...
    def validate_sales_type(self, key, sales_type):
        ...
        if isinstance(sales_type, str):
            try:
                ...
                raise ValueError(
                    f"Sales type must be '{SalesTypeEnum.RENT.value}' or '{SalesTypeEnum.SALE.value}'"
                )
    ...
    def validate_property_type(self, key, property_type):
        ...
        if isinstance(property_type, str):
            try:
                ...
                raise ValueError(
                    f"Property type must be '{PropertyTypeEnum.APARTMENT.value}' or '{PropertyTypeEnum.HOUSE.value}'"
                )
    ...
    def get_sales_type_display(self) -> str:
        ...
        sales_names = {SalesTypeEnum.RENT: "Rent", SalesTypeEnum.SALE: "Sale"}
