...
class Visit(BaseModel):
    ...
    # Visit attributes
    visitDate = Column(DateTime(timezone=True), nullable=False, index=True)

    isVisitCompleted = Column(Boolean, default=False, nullable=False)
    ...
    def can_be_completed(self) -> bool:
        ...
        return (
            not self.cancelled
            and not self.isVisitCompleted
            and self.visitDate <= datetime.now(timezone.utc)
        )

    def can_be_cancelled(self) -> bool:
        """Check if visit can be cancelled"""
        return not self.cancelled and not self.isVisitCompleted and self.is_upcoming()
