...
async def get_my_proposals(
    ...
    status: Optional[str] = Query(
        None, description="Filter by status: pending, accepted, rejected, withdrawn"
    ),
    current_user: AuthUser = Depends(get_current_user),
    db: Session = Depends(get_db),
):
    ...
    **Returns:** Paginated list of your proposals with status and tracking information
    ...
    proposals, total = proposal_service.get_user_proposals(
        user_id=current_user.id, page=page, size=size, status=status
    )
...
async def get_received_proposals(
    ...
    **Security:** The auth service validates that property owners can only access
    ...
    proposals, total = proposal_service.get_received_proposals(
        owner_id=current_owner.id, page=page, size=size, status=status
    )
    ...
    for proposal in proposals:
        ...
        property_obj = (
            db.query(Property).filter(Property.id == proposal.idProperty).first()
        )
        ...
        if property_obj:
            ...
            address_str = (
                f"{property_obj.address.street}, {property_obj.address.number} - {property_obj.address.city}"
        ...
        proposal_response = ProposalWithUserResponse.from_proposal(
            proposal, user_info, property_info
        )
...
@router.get(
    "/{proposal_id}", response_model=ProposalResponse, summary="Get Proposal Details"
)
