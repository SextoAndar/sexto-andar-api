...
async def user_deleted_webhook(
    ...
    **Returns:**
    ...
    try:
        ...
        deleted_favorites_count = favorite_service.delete_all_user_favorites(
            request.user_id
        )
        logger.info(
            f"Deleted {deleted_favorites_count} favorites for user {request.user_id}."
        )

        # Delete all visits made by this user
        deleted_user_visits_count = visit_service.delete_all_user_visits(
            request.user_id
        )
        logger.info(
            f"Deleted {deleted_user_visits_count} user visits for user {request.user_id}."
        )

        # Delete all visits for properties owned by this user
        deleted_owner_prop_visits_count = (
            visit_service.delete_all_visits_for_owner_properties(request.user_id)
        )
        logger.info(
            f"Deleted {deleted_owner_prop_visits_count} visits for properties of owner {request.user_id}."
        )

        # Delete all proposals made by this user
        deleted_user_proposals_count = proposal_service.delete_all_user_proposals(
            request.user_id
        )
        logger.info(
            f"Deleted {deleted_user_proposals_count} user proposals for user {request.user_id}."
        )

        # Delete all proposals for properties owned by this user
        deleted_owner_prop_proposals_count = (
            proposal_service.delete_all_proposals_for_owner_properties(request.user_id)
        )
        logger.info(
            f"Deleted {deleted_owner_prop_proposals_count} proposals for properties of owner {request.user_id}."
        )

        logger.info(
            f"Cascading deletion completed successfully for user_id: {request.user_id}"
        )

    except HTTPException as e:
        logger.error(
            f"Error during cascading deletion for user_id {request.user_id}: {e.detail}"
        )
        raise  # Re-raise the HTTPException
    except Exception as e:
        logger.error(
            f"Unhandled error during cascading deletion for user_id {request.user_id}: {e}"
        )
        raise HTTPException(
    ...
    return {
        "message": f"Webhook processed and cascading deletion initiated for user_id: {request.user_id}"
    }
