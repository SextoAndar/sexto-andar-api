...
async def get_my_properties(
    ...
    property_type: Optional[str] = Query(
        None, description="Filter by type: 'HOUSE' or 'APARTMENT'"
    ),
    sales_type: Optional[str] = Query(
        None, description="Filter by sales type: 'RENT' or 'SALE'"
    ),
...
@router.get(
    "/{property_id}", response_model=PropertyResponse, summary="Get Property Details"
)
...
async def list_properties(
    ...
    search_term: Optional[str] = Query(
        None, description="Text search for property description and address"
    ),
    active_only: bool = Query(True, description="Show only active properties"),
    db: Session = Depends(get_db),
):
...
@router.put(
    "/{property_id}", response_model=PropertyResponse, summary="Update Property Details"
)
async def update_property(
    ...
    **Error responses:**
    ...
    updated_property = property_service.update_property(
        property_id=property_id, update_data=update_data, owner_id=current_owner.id
    )
...
@router.delete(
    "/{property_id}", status_code=status.HTTP_204_NO_CONTENT, summary="Delete Property"
)
...
async def activate_property(
    ...
    **Error responses:**
    ...
    activated_property = property_service.activate_property(
        property_id=property_id, owner_id=current_owner.id
    )
