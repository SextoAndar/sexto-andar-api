...
@router.get(
    "/my-visits", response_model=VisitListResponse, summary="View My Scheduled Visits"
)
...
@router.get(
    "/upcoming", response_model=list[VisitResponse], summary="Get Upcoming Visits"
)
...
async def complete_visit(
    ...
    **Returns:** Updated visit with completed status
    ...
    visit = visit_service.complete_visit(
        visit_id, current_user.id, completion_data.notes
    )
...
@router.post(
    "/{visit_id}/cancel", response_model=VisitResponse, summary="Cancel a Visit"
)
async def cancel_visit(
    ...
    **Returns:** Updated visit with cancelled status
    ...
    visit = visit_service.cancel_visit(
        visit_id, current_user.id, cancellation_data.cancellation_reason
    )
...
@router.delete(
    "/{visit_id}", status_code=status.HTTP_204_NO_CONTENT, summary="Delete a Visit"
)
...
async def get_property_visits(
    ...
    **Returns:** Paginated list of visits for the property
    ...
    visits, total = visit_service.get_property_visits(
        property_id=property_id, page=page, size=size, include_cancelled=False
    )
