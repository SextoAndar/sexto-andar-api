...
class DataSeeder:
    ...
    def create_property(
        self, token: str, property_type: str, data: Dict
    ) -> Optional[Dict]:
        ...
        try:
            ...
            if response.status_code == 201:
                ...
                self.log_success(
                    f"Created {property_type}: {data['description'][:50]}..."
                )
                return property_data
            else:
                self.log_error(
                    f"Failed to create property: {response.status_code} - {response.text}"
                )
                return None
    ...
    def create_visit(
        self, token: str, property_id: str, visit_date: str, notes: str = None
    ) -> Optional[Dict]:
        """Create a visit"""
        try:
            url = f"{self.api_url}/visits/"
            cookies = {"access_token": token}
            data = {"idProperty": property_id, "visitDate": visit_date, "notes": notes}
            ...
            if response.status_code == 201:
                ...
                self.log_success(
                    f"Created visit for property {property_id[:8]}... on {visit_date}"
                )
                return visit_data
            else:
                self.log_error(
                    f"Failed to create visit: {response.status_code} - {response.text[:200]}"
                )
                return None
    ...
    def create_proposal(
        self, token: str, property_id: str, value: float, message: str = None
    ) -> Optional[Dict]:
        """Create a proposal"""
        try:
            ...
            if response.status_code == 201:
                ...
                self.log_success(
                    f"Created proposal for property {property_id[:8]}... - ${value:,.2f}"
                )
                return proposal_data
            else:
                self.log_error(
                    f"Failed to create proposal: {response.status_code} - {response.text[:200]}"
                )
                return None
    ...
    def seed_properties(self) -> bool:
        ...
        self.log_info(
            f"Successfully logged in {sum(1 for o in owners if 'token' in o)}/{len(owners)} property owners"
        )
        ...
        for i, apartment in enumerate(apartments):
            ...
            if "token" in owner:
                ...
                self.log_info(
                    f"Apartment {i + 1}: Added {len(apartment['images'])} images"
                )
                property_data = self.create_property(
                    owner["token"], "apartment", apartment
                )
                if property_data:
                    self.created_properties.append(property_data)
    ...
    def seed_visits(self) -> bool:
        ...
        for i, property_data in enumerate(self.created_properties):
            ...
            selected_users = random.sample(
                [u for u in users if "token" in u], num_visits
            )

            for j, user in enumerate(selected_users):
                # Create datetime with timezone
                visit_datetime = base_date + timedelta(
                    days=i * 2 + j, hours=10
                )  # 10 AM
                ...
                visit = self.create_visit(
                    user["token"], property_data["id"], visit_date, notes
                )
        ...
        for i, property_data in enumerate(self.created_properties):
            ...
            for user in selected_users:
                ...
                proposal = self.create_proposal(
                    user["token"], property_data["id"], proposal_value, message
                )
