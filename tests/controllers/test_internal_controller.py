...
def test_user_deleted_webhook_success(
    ...
    response = client.post(
        "/api/internal/user-deleted-webhook", json=payload, headers=headers
    )

    assert response.status_code == 200
    assert (
        response.json()["message"]
        == f"Webhook processed and cascading deletion initiated for user_id: {test_user_id_uuid}"
    )

    # Assert that all service deletion methods were called with the correct user_id
    mock_property_service.delete_all_owner_properties_permanently.assert_called_once_with(
        test_user_id_uuid
    )
    mock_favorite_service.delete_all_user_favorites.assert_called_once_with(
        test_user_id_uuid
    )
    mock_visit_service.delete_all_user_visits.assert_called_once_with(test_user_id_uuid)
    mock_visit_service.delete_all_visits_for_owner_properties.assert_called_once_with(
        test_user_id_uuid
    )
    mock_proposal_service.delete_all_user_proposals.assert_called_once_with(
        test_user_id_uuid
    )
    mock_proposal_service.delete_all_proposals_for_owner_properties.assert_called_once_with(
        test_user_id_uuid
    )
