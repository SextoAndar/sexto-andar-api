...
class TestPropertySalesTypeValidation:
    ...
    def test_invalid_sales_types(self, sales_type, test_house_data):
        ...
        assert (
            "sales type" in str(exc_info.value).lower()
            or "value error" in str(exc_info.value).lower()
        )
...
class TestApartmentSpecificValidation:
    ...
    def test_valid_condominium_fees(
        self, condominium_fee, expected, test_apartment_data
    ):
