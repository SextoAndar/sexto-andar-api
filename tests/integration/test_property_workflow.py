...
class TestPropertyLifecycle:
    ...
    def test_create_update_delete_workflow(self, db_session: Session, test_house_data):
        ...
        update_data = UpdatePropertyRequest(
            description="Updated description after creation", propertyValue="600000.00"
        )
        updated = service.update_property(str(created.id), update_data, owner_id)
    ...
    def test_multiple_properties_same_owner(
        self, db_session: Session, test_house_data, test_apartment_data
    ):
...
class TestPropertyStatistics:
    ...
    def test_statistics_with_inactive_properties(
        self, db_session: Session, test_house_data
    ):
...
class TestPropertyPagination:
    ...
    def test_pagination_second_page(self, db_session: Session, test_house_data):
        ...
        # Get second page
        properties, total = service.get_properties_by_owner(owner_id, page=2, size=10)
    ...
    def test_different_page_sizes(
        self, db_session: Session, test_house_data, page_size
    ):
...
class TestPropertyOwnership:
    ...
    def test_non_owner_cannot_update_property(
        self, db_session: Session, test_house_data
    ):
        ...
        with pytest.raises(Exception) as exc_info:
            service.update_property(str(created.id), update_data, other_user_id)
    ...
    def test_admin_can_delete_any_property(self, db_session: Session, test_house_data):
        ...
        service.delete_property(str(created.id), admin_id, is_admin=True)
