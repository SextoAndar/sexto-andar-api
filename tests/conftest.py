...
os.environ.setdefault(
    "DATABASE_URL",
    "postgresql://sexto_andar_user:sexto_andar_pass@localhost:5432/sexto_andar_test_db",
)
...
def mock_auth_user():
    ...
    return AuthUser({"sub": str(uuid4()), "username": "testuser", "role": "USER"})
...
def mock_auth_property_owner():
    ...
    return AuthUser(
        {"sub": str(uuid4()), "username": "testowner", "role": "PROPERTY_OWNER"}
    )
...
def mock_auth_admin():
    ...
    return AuthUser({"sub": str(uuid4()), "username": "testadmin", "role": "ADMIN"})
...
def authenticated_property_owner(client: TestClient, mock_auth_property_owner):
    ...
    app.dependency_overrides[get_current_property_owner] = (
        override_get_current_property_owner
    )
    app.dependency_overrides[get_current_user] = override_get_current_user

    yield {"owner": mock_auth_property_owner, "client": client}
...
def authenticated_user(client: TestClient, mock_auth_user):
    ...
    yield {"user": mock_auth_user, "client": client}
...
def authenticated_admin(client: TestClient, mock_auth_admin):
    ...
    yield {"admin": mock_auth_admin, "client": client}
...
def created_property(db_session: Session, mock_auth_property_owner, test_house_data):
    ...
    property_obj = property_service.create_house(
        house_request, mock_auth_property_owner.id
    )
