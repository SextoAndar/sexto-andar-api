...
class TestPropertyRepositoryParametrized:
    ...
    @pytest.mark.parametrize(
        "property_type,sales_type,value",
        [
    ...
    def test_pagination_scenarios(self, db_session: Session, page, size, expected_min):
    ...
    @pytest.mark.parametrize(
        "property_size,expected_decimal",
        [
            ("50.00", Decimal("50.00")),
...
class TestPropertyFilteringParametrized:
    ...
    @pytest.mark.parametrize(
        "filter_type,filter_value,expected_match",
        [
            (
                "property_type",
                PropertyTypeEnum.HOUSE,
                lambda p: p.propertyType == PropertyTypeEnum.HOUSE,
            ),
            (
                "property_type",
                PropertyTypeEnum.APARTMENT,
                lambda p: p.propertyType == PropertyTypeEnum.APARTMENT,
            ),
            (
                "sales_type",
                SalesTypeEnum.SALE,
                lambda p: p.salesType == SalesTypeEnum.SALE,
            ),
            (
                "sales_type",
                SalesTypeEnum.RENT,
                lambda p: p.salesType == SalesTypeEnum.RENT,
            ),
        ],
    )
...
class TestAddressRepositoryParametrized:
    ...
    def test_different_postal_codes(self, db_session: Session, postal_code):
